...
We've discovered that something's off in the signup flow. Students can currently register for the same activity **more than once**! Let's see how far Copilot can take us in uncovering the cause and shaping a clean fix.
...
- **How do I make prompts?:** Being explicit and clear helps Copilot do the best job. But unlike some traditional systems, you can always clarify your direction with follow-up prompts.
...
### :keyboard: Activity: Use Copilot to fix our registration bug 🐛
...
1. Now that we know the issue is in the `src/app.py` file and the `signup_for_activity` function, let's follow Copilot's recommendation and go fix it (semi-manually). We'll start with a comment and let Copilot finish the correction.
   ...
   1. Enter the below comment and press enter to go to the next line. After a moment, temporary shadow text will appear with a suggestion from Copilot! Nice! 🎉
      ...
      <img width="700" alt="Copilot shadow text suggestion in the editor" src="https://github.com/{{full_repo_name}}/raw/main/src/static/images/shadow-text.gif" />
...
1. Near the top of the `src/app.py` file (about line 27), find the `activities` variable, where our example extracurricular activities are configured.
...
1. After a moment, Copilot will directly start making changes to the code. The changes will be stylized differently to make any additions and removals easy to identify. Take a moment to inspect and verify the changes, and then press the **Keep** button.
   ...
   activities = {
       ...
       "Football Team": {
           "description": "Competitive football training and friendly matches across Amsterdam",
           "schedule": "Mondays and Fridays, 5:00 PM - 7:00 PM",
           "max_participants": 22,
           "participants": []
       },
       "Swimming Club": {
           "description": "Swim training sessions at the university pool facility",
           "schedule": "Tuesdays and Thursdays, 7:00 AM - 8:30 AM",
           "max_participants": 20,
           "participants": []
       },
       "Photography Circle": {
           "description": "Learn urban photography exploring Amsterdam's architecture and street life",
           "schedule": "Saturdays, 2:00 PM - 4:00 PM",
           "max_participants": 12,
           "participants": []
       },
       "Theatre Workshop": {
           "description": "Improvisational and script-based acting in an international environment",
           "schedule": "Thursdays, 6:00 PM - 8:30 PM",
           "max_participants": 18,
           "participants": []
       },
       "Philosophy Debate Club": {
           "description": "Weekly debates on contemporary ethical and philosophical questions",
           "schedule": "Mondays, 4:00 PM - 5:30 PM",
           "max_participants": 20,
           "participants": []
       },
       "Economics Research Group": {
           "description": "Student-led research on European economic trends and policy",
...
1. Find the `app.py` file and press the `+` sign to collect your changes together in the staging area.
   ...
   <img width="400" alt="Staging changes icon in VS Code Source Control" src="https://github.com/{{full_repo_name}}/raw/main/src/static/images/staging-changes-icon.png" />
...
- Make sure you pushed the `src/app.py` file changes to the branch `accelerate-with-copilot`.
